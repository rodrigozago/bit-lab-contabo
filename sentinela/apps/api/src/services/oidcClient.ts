...
export async function finishAuth(callbackParams: Record<string, string>): Promise<AuthClaims> {
  ...
  // Claims de scope (email, is_admin) vêm do endpoint USERINFO, não do ID
  // token — mesma observação do ponto-studio/face-lab: com userinfo
  // habilitado, o ID token só carrega o sub. is_admin aqui é o superuser
  // central do bit-lab (auth/src/models/users.js), não um papel do tenant —
  // usado só pra liberar o dashboard de scraping (rotas /api/admin/*).
