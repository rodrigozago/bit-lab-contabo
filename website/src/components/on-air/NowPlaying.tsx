...
import { fmtRange, formatInstagramHandle } from "@/lib/onair-format";

export function NowPlaying({ slot }: { slot: OnAirSlotView | null }) {
  return (
        ...
        {slot ? (
          <>
            ...
            <p className="mt-5 text-xl tabular-nums text-muted">
              {fmtRange(slot.startsAtIso, slot.endsAtIso)}
            </p>
            {slot.instagramUrl && (
              <div className="mt-7 flex items-center justify-center gap-4 lg:justify-start">
                ...
                <div className="shrink-0 rounded-xl bg-white p-2.5">
                  <QRCodeSVG value={slot.instagramUrl} size={112} fgColor="#060b1d" bgColor="#ffffff" />
                </div>
                <div className="text-left">
                  ...
                  <p className="mt-1 font-bold text-accent-bright">
                    {formatInstagramHandle(slot.instagramRaw ?? slot.instagramUrl)}
                  </p>
                </div>
              </div>
            )}
